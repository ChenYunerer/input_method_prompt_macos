...
当前为本地 1.11.3 / 构建 35：补齐键盘设备列表更新。旧客户端读数合法但列表缺失时也会刷新，避免后接入蓝牙键盘的大写锁定被漏报。客户端使用满 0.5 秒后的首次查询会重建客户端，保留公开锁定属性读取、P0 前三项、防闪与未知状态处理。
...
复用一个由 ARC 管理的简单 HID 客户端；每次枚举当前服务，只读取符合 Generic Desktop / Keyboard 用途的服务。属性必须是 CFBoolean，不把数字或字符串强转为布尔。任一键盘返回 true 则为开启；非空列表全部明确 false 才为关闭；列表为空、枚举失败，或无 true 且存在未知属性时返回 nil。不依赖品牌、名称或固定顺序。实测同一个客户端可能保留不可读服务，也可能只保留少数可读服务而遗漏后接入键盘，因此在客户端使用满 0.5 秒后的首次查询中先重建再读取。查询闭包持有客户端，替换闭包释放旧客户端；窗口内继续实时读取各服务状态，不缓存布尔结果。暂停很久后只重建一次，并以当前时间设置下一期限，不追补过期周期。快速轮询遇到 nil 取消候选并进入原有单次恢复流程，不频繁读取 TIS。确认截止时再次查询，100/250 ms 规则过滤短暂变化。
...
## 23. 长期运行后切换提示失效（1.11.2 / build 34）

### 23.1 现场与根因

2026-09-23，用户反馈切换无提示，鼠标旁标识也不变化。安装进程仍是 build 33，已运行约 18 小时；提示开关开启，线程采样显示主循环和定时器正常工作，并未卡死。新建独立监测器可连续确认中文、英文变化，真实运行进程界面停留在旧状态。

保留旧进程后，通过调试器只调用其既有键盘查询闭包，结果为 `[nil, false, nil]`；新进程以同样接口读取三个键盘服务均为 `false`。调试器退出并分离后，原进程继续运行。问题在于 build 33 的客户端被查询闭包长期持有：虽然每次调用 `CopyServices`，仍不能恢复已经不可读的服务。Caps Lock 返回未知使完整 `InputState` 读取失败，所有提示停止更新；0.5 秒状态重试仍复用同一个坏客户端，因而无法自愈。

系统记录包含此前的休眠/唤醒，但没有捕获首次连接失效的瞬间；不能据此认定某次休眠或某个键盘是唯一触发条件。现场记录位于忽略目录 `app/.build/caps-lock-investigation/20260923/`，不记录输入内容。

### 23.2 修复

`CapsLockReader` 改为保存可重建的查询闭包工厂，健康时复用客户端。读取出现枚举失败、空列表或任一键盘属性未知时，在距上次创建至少 0.5 秒后新建客户端并立即重读，每次调用最多重建一次。新闭包替换旧闭包使旧客户端通过 ARC 释放；持续不可用时不会在每 80 ms 轮询中反复重建。

冷却期依然读取当前客户端，因此接口自行恢复后无需等待冷却结束。最终布尔汇总和未知语义保持不变：任一明确开启即开启，非空列表全部关闭才确认关闭；不回退至旧事件标志，不将 nil 当作 false。所有恢复结果仍通过现有 100/250 ms 防闪确认，P0 三项不变。

回归注入与现场一致的旧 `[nil, false, nil]` / 新全 false，验证重建、旧对象释放、健康复用、失败限频、启动不可用恢复，以及监测器中英文提示恢复后真实大写开关正常。


### 23.3 验证与交付

Caps Lock 专项 58 项、全量 409 项通过，均为 0 失败；arm64 release 构建、签名和基础状态诊断通过。故障注入验证客户端重建后恢复状态发布，但不等同于完整重演一次过夜休眠。

已将 1.11.2 / build 34 替换到 `/Applications/中英提示.app` 并重启，旧 build 33 保留备份。确认仅单个新进程，安装二进制与构建产物 SHA-256 相同，11 项偏好保持一致；界面读取到鼠标标识已从旧的 `a` 更新为当前 `中`。安装回执位于忽略目录 `app/.build/local-install/20260923-104209-keyboard-client-recovery/receipt.json`。只读采样及调试器均已结束，无新增后台诊断进程。

2026-09-23，安装后的用户切换回验确认：“两处都恢复了”，屏幕中央提示与鼠标旁标识均正常更新。下一次休眠/设备重连及长期运行仍需继续验证。


## 24. 蓝牙键盘大写漏报（1.11.3 / build 35）

### 24.1 根因证据

用户报告蓝牙键盘实际锁定大写时无提示，内置键盘正常。只读对照在 17:38:18–17:38:30 捕获到蓝牙服务明确返回 true，独立监测器也确认了 A，说明公开属性能够识别这台蓝牙键盘，并非仅支持内置键盘。

随后保留正在运行的 build 34 进程，通过调试器分别调用同一读取器的旧查询闭包和新建查询工厂：旧返回 `[false]`，新返回 `[false, false, false]`。此时验证的是设备列表差异，不是锁定已开启；新查询未替换旧进程的持有对象。记录位于忽略目录 `app/.build/caps-lock-investigation/20260923-bluetooth/runtime/installed-comparison.txt`。

build 34 的重连条件只覆盖空列表、枚举失败和未知属性；仅返回一个有效 false 的旧列表被当作健康，因而永远发现不了遗漏的键盘。第 23 节修复处理了不可读服务，尚未处理这种有效但不完整的列表。对照采样中刚创建客户端与保留客户端的短暂差异包含 0.5 秒采样时差，不能将其直接当作布尔值缓存的证据；本轮依据是同一正式进程的设备数量差异。

### 24.2 实现与代价

`CapsLockReader` 将故障重试期限改为通用客户端刷新期限。刷新间隔设为 0.5 秒；下一次查询发现到期时，只重建一次再读取，重新设置期限。查询没有独立新增计时器，复用既有 80 ms 轮询；到期前仍查询服务属性，大小写变化不必等到下次重建。

这样既发现后接入设备，也移除旧列表中仍返回 true 的断开设备。无品牌判断、无新权限、无事件标志兜底；未知状态、Caps Lock 100/250 ms 确认与 P0 三项保持不变。新增键盘最坏需等待一次客户端刷新及正常确认窗口，受主线程调度影响，不保证实时期限。

本机各 100 次查询微基准：复用查询中位 0.179 ms / P95 0.446 ms；新建客户端并查询中位 1.867 ms / P95 2.169 ms，最大 3.509 ms。最多每秒两次重建，未在每次 80 ms 轮询中重建。这是局部调用耗时，不是整机 CPU 或能耗验收。

### 24.3 回归

新增“仅有内置服务 → 接入已锁定的蓝牙服务”的列表快照模拟，旧代码出现 9 项失败；同时覆盖窗口内复用、同值读取不重复创建、移除键盘后清除旧 true、休眠后不追补刷新、无通知时自动发现，以及发现后开关大写和内置键盘正常识别。

修复后 Caps Lock 专项 69 项、全量复测 420 项通过，0 失败。首次全量测试有 1 项“原来的前台应用保持不变”检查失败，未采集当时的焦点变化，原因不能确认。保留原断言，补充只读焦点观察后独立浮层自检通过，全量复测也通过；独立自检期间前台 PID 始终相同。原始结果与复测记录位于忽略目录 `app/.build/caps-lock-investigation/20260923-bluetooth/`。

arm64 release 构建、签名校验及基础 `--diagnose` 通过。已将 `dist/keyboard-discovery-fix/中英提示.app`（1.11.3 / build 35）替换到 `/Applications/中英提示.app` 并重启，确认单个新进程、安装二进制与构建产物 SHA-256 一致、11 项偏好保持一致，旧 build 34 保留备份。安装回执为 `app/.build/local-install/20260923-182722-keyboard-discovery-fix/receipt.json`。

2026-09-24，用户完成蓝牙键盘断开重连及实体开启、关闭大写锁定的安装后回验，反馈“两处都正常了”，确认屏幕中央提示和鼠标旁标识均正确变化。本机该场景已通过；长期运行、完整多设备兼容与整机能耗尚未完成验收。
